...
- Plataforma objetivo: Windows, macOS y Linux.
...
Hay tres pruebas de integración ignoradas por defecto: PostgreSQL, MySQL y MongoDB. Requieren variables `RUSIODB_PG_*`, `RUSIODB_MYSQL_*` o `RUSIODB_MONGO_*` con `HOST`, `PORT`, `DATABASE`, `USER`, `PASSWORD` y `TLS`; la de Mongo además necesita `RUSIODB_MONGO_COLLECTION` con una colección existente.
...
9. Verificar empaquetado e interacción real en macOS y Linux.
